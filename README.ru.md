...
при подъёме вверх от текущего каталога (останавливаясь на `$HOME`) →
`~/.config/tgc`. Общий родительский `workspace/.tgc` покрывает все подпроекты;
более близкий `./.tgc` его переопределяет.
